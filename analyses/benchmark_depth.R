...
plot_data <- df %>%
  ...
  mutate(mean_depth_scores = round(mean_depth_scores, 1)) %>%
  # Filter out values > 5
  filter(mean_depth_scores <= 5) %>%
  filter(mean_depth_scores >= 1) %>%
  # Get best score per dataset and expected test value
  group_by(data__keyword, model_name, mean_depth_scores) %>%
  summarise(
...
ggplot(plot_data, aes(x = mean_depth_scores, y = mean_test_score, color = model_name)) +
  ...
  geom_text_repel(aes(label = model_name),
                  data = plot_data %>% 
                    group_by(model_name) %>% 
                    filter(mean_depth_scores == min(mean_depth_scores)),
                  bg.color = 'white', 
                  size = 6.5, 
                  bg.r = 0.15,
                  nudge_y = 0, 
  ...
  scale_x_continuous(name = "Tree Depth") +
  scale_y_continuous(name = "Mean Test Score") +
  theme_minimal(base_size = 22) +
  theme(legend.position = "none") +
  colScale

ggsave("analyses/plots/benchmark_depth_numerical_classif.pdf", width=7, height=6, bg="white")
...
ggsave("analyses/plots/benchmark_depth_categorical_classif.pdf", width=7, height=6, bg="white")