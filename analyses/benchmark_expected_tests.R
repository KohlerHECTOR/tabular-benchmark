...
df <- read_csv("src/results.csv") %>%
  ...
  select(model_name, data__keyword, mean_val_score, mean_test_score, mean_time, hp, mean_expected_tests_scores) %>% 
...
plot_data <- df %>%
  ...
  mutate(mean_expected_tests_scores = round(mean_expected_tests_scores, 1)) %>%
  # Filter out values > 5
  filter(mean_expected_tests_scores <= 5) %>%
  filter(mean_expected_tests_scores >= 1) %>%
  # Get best score per dataset and expected test value
  group_by(data__keyword, model_name, mean_expected_tests_scores) %>%
  summarise(
    best_score = max(mean_test_score, na.rm = TRUE),
    .groups = 'keep'
  ) %>%
  # Now average the best scores across datasets
  group_by(model_name, mean_expected_tests_scores) %>%
  summarise(
...
ggplot(plot_data, aes(x = mean_expected_tests_scores, y = mean_test_score, color = model_name)) +
  geom_line(size = 2) +
  ...
  geom_text_repel(aes(label = model_name),
                  data = plot_data %>% 
                    group_by(model_name) %>% 
                    filter(mean_expected_tests_scores == min(mean_expected_tests_scores)),
                  bg.color = 'white', 
                  size = 6.5, 
                  bg.r = 0.15,
                  nudge_y = 0, 
  ...
  scale_x_continuous(name = "Expected Number of Tree Tests") +
  scale_y_continuous(name = "Mean Test Score") +
  theme_minimal(base_size = 22) +
  theme(legend.position = "none") +
  colScale

ggsave("analyses/plots/benchmark_exp_tests_numerical_classif.pdf", width=7, height=6, bg="white")