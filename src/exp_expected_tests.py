...
grouped_stats = df.groupby('mean_depth_scores')['mean_test_score'].mean().reset_index()

df_dpdt = pd.read_csv("results_cart_dpdt.csv")
# df_dpdt = df_dpdt[df_dpdt["mean_depth_scores"] >= 1]
...
# df_cart = df_cart[df_cart["mean_depth_scores"] >= 1]
...
plt.tight_layout()
plt.savefig("res_depth_numerical.pdf")