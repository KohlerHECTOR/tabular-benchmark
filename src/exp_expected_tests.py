...
# df = df[df["mean_expected_tests_scores"] >= 1]
...
model_params = [col for col in df.columns if col.startswith('model__') or col == "mean_expected_tests_scores"]
...
grouped_stats = df.groupby('mean_expected_tests_scores')['mean_test_score'].max().reset_index()

df_dpdt = pd.read_csv("results_cart_dpdt.csv")
# df_dpdt = df_dpdt[df_dpdt["mean_expected_tests_scores"] >= 1]
...
# Get hyperparameter columns (model parameters)
model_params_dpdt = [col for col in df_dpdt.columns if col.startswith('model__') or col == "mean_expected_tests_scores"]
...
# Group by expected_tests_scores and get max test_score
grouped_stats_dpdt = df_dpdt.groupby('mean_expected_tests_scores')['mean_test_score'].max().reset_index()


df_cart = pd.read_csv("results_cart_dpdt.csv")
# df_cart = df_cart[df_cart["mean_expected_tests_scores"] >= 1]
...
# Get hyperparameter columns (model parameters)
model_params_cart = [col for col in df_cart.columns if col.startswith('model__') or col == "mean_expected_tests_scores"]
...
# Group by expected_tests_scores and get max test_score
grouped_stats_cart = df_cart.groupby('mean_expected_tests_scores')['mean_test_score'].max().reset_index()
