...
df_copy = pd.read_csv("results.csv")
...
always_fast_mask = df.groupby(model_params)['mean_time'].transform(lambda x: x.max() <= 3)

always_fast_combinations = df[always_fast_mask][model_params].drop_duplicates()

print("Parameter combinations that NEVER resulted in mean_time > 3 seconds:")
...
print(always_fast_stats[always_fast_stats['count']>= 10].sort_values('mean', ascending=True))
...
print(np.array(to_proba)/np.array(to_proba).sum())
